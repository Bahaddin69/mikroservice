...
import { MessageType, CargoEvent, TOPIC_TYPE } from "../../types";

const CLIENT_ID = process.env.CLIENT_ID || "cargo-service";
const GROUP_ID = process.env.GROUP_ID || "cargo-service-group";
...
const subscribe = async (
    ...
    await consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
            if (topic !== "CargoEvents")
                return;

            if (message.key && message.value) {
                const inputMessage: MessageType = {
                    headers: message.headers,
                    event: message.key.toString() as CargoEvent,
